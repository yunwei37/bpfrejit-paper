...
%%%
% message<overview of the post-load recompilation loop>
...
% Section 4.2: Challenges and Insights
...
% Section 4.3: Choosing and Revisiting Whole-Program Rewrites

%%%
% message<choosing rewrites is a userspace policy decision>
% profitable rewrites depend on workload, machine, deployment state, and execution phase
% these choices should not be frozen into a fixed in-kernel policy
% the daemon decides whether to revisit a live program and which candidate to try
% the kernel remains the final authority on legality and safety

%%%
% message<the daemon submits one whole-program candidate rather than incremental patches>
% multiple rewrites interact and should be composed before kernel admission
% normalization, kinsn-introducing rewrites, and runtime specialization are combined in userspace
% the kernel makes one accept-or-reject decision over the composed result
% this keeps the recompilation interface stable as new rewrites are added

%%%
% message<recompilation is repeatable because each round restarts from the same baseline>
% runtime facts may change after deployment and justify another recompilation round
% each round starts from saved original bytecode rather than from an already rewritten image
% rejection leaves the current live program unchanged
% repeated recompilation reuses the same trusted kernel admission path
...
\begin{figure}[hbtp]
    \centering
    \includegraphics[width=1\linewidth]{figures/sec-4-arch.pdf}
